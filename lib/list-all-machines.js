...
var History = require('./historyManager');
...
function listForAccount( account, callback ){
        ...
        clouds[account.type].list( account , function( err , result ){
            if ( !!err ){
                logger.error('unable to list machines for ', account.description, err );
                callback(err);
            }
...
exports.list = function( config, callback ) {
    ...
    async.each( conf.accounts, function(account, callback ){
        ...
        function( err ){
            ...
            var report = { 'total' : total ,'timestamp' : new Date().getTime(), 'summary' : summary, 'details' : results};


            var history = new History(conf.history);
            function finished(){
            ...
            try {
                history.saveReport(report, finished);
            }catch(e){
